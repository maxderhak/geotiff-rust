...
- stop synthesizing the `ExtraSamples` tag (338) from photometric for **any** interpretation — the writer now emits only the extras the caller explicitly declares via `ImageBuilder::extra_samples(...)`. Previously the writer derived a fixed base sample count per photometric (1 for `MinIsBlack`/`MinIsWhite`/`Palette`/`Mask`, 3 for `Rgb`/`YCbCr`/`CieLab`/`IccLab`, 4 for `Separated` `InkSet::Cmyk`) and PADDED the declared extra-samples list to `samples_per_pixel - base` `Unspecified` codes, writing a spurious `ExtraSamples` tag the caller never asked for. This surfaced for Onyx's spectral rasters — `MinIsBlack`/`MinIsWhite` with N samples where every sample is a spectral band (base image data, no extras) got a bogus `N-1`-entry tag — but the same synthesis applied to `Rgb` spp=4 (1 synthesized), `Separated` CMYK spp=6 (2 synthesized), etc. Now no photometric pads: a caller that declares no extras writes **no** `ExtraSamples` tag, and one that declares extras writes exactly those codes. The per-photometric over-declaration guard is preserved (declared extras may not exceed `samples_per_pixel - base`, so e.g. `Rgb` spp=4 still rejects >1 declared extra) — it is not loosened to a flat `<= samples_per_pixel`. This matches libtiff, which writes `EXTRASAMPLES` solely when the application sets `TIFFTAG_EXTRASAMPLES` (`tif_dirwrite.c` gates the field on `FIELD_EXTRASAMPLES`; the tag is never derived from photometric) and defaults excess channels to `EXTRASAMPLE_UNSPECIFIED` on read. The reader is **unchanged** — it already tolerates an absent tag and defaults excess channels to `Unspecified` in memory, so every write→read round-trip stays pixel-identical. Note: a write client that previously relied on the writer synthesizing extras (e.g. `Separated` CMYK with spot/HiFi channels, or `Rgb`+alpha) must now declare its real extras explicitly
- extend the verbatim (zero-copy, padding-bit-exact) packed sub-byte read from full-width windows to **every byte-aligned column sub-window**. A `col_off > 0` / partial-width sub-byte window is now returned verbatim — the exact on-disk bytes, trailing padding bits included — whenever its byte range is a contiguous whole-byte sub-range of the packed row: the left edge starts on a byte boundary (`col_off * samples_per_pixel * bits` a multiple of 8 for a chunky interleaved row, or `col_off * bits` for a planar sample-plane band) **and** the right edge either ends on a byte boundary or runs to the image width (so the final byte's spare bits are the row's own trailing padding, never a real sample from a column to the right of the window). This covers, in particular, byte-aligned windows anchored at the image edge (e.g. `col_off = 8, cols = 5` for a 13-wide 1-bit chunky row), which previously re-packed and zero-filled the on-disk padding. Genuinely **bit-granular** windows (a mid-byte `col_off`, or a partial-width window ending mid-byte before the image edge) and all **tiled** sub-byte storage keep re-packing the windowed unpacked decode fresh from bit 0 (padding zeroed, pixels correct) — a per-band chunky (bit-interleaved) window likewise stays on the re-pack path, since a single band out of an interleaved chunky row is never a whole-byte on-disk run. A shared `packed_col_window_is_byte_aligned` predicate encodes the rule and the packed strip copy now takes a byte offset + output row width so the verbatim path slices the window's byte sub-range out of each full-width decoded row. The packed accessors' rustdoc now states precisely which windows are verbatim (padding-exact) vs re-packed (padding zero-filled), and the `col_off`/`cols` re-pack note moved from `read_band_packed_bytes` (which takes no `col_off`) to `read_band_window_packed_bytes` (which does). The chunky verbatim branch gained a `debug_assert!(planar_configuration == 1)` documenting its ordering-coupled reliance on the planar-sub-byte rejection above it. No change to the unpacked/decoded API, pixel/Lab semantics, or the planar-non-band rejection guard
