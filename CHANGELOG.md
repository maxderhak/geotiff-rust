...
- read raw on-disk **packed** storage bytes: `TiffFile` gains `read_image_packed_bytes`, `read_window_packed_bytes`, `read_band_packed_bytes`, `read_band_window_packed_bytes` (and their `*_from_ifd` variants), paralleling the existing unpacked `*_bytes` accessors. For sub-byte depths (1/2/4-bit) these return the packed, MSB-first representation — `ceil(cols * samples_per_pixel * bits / 8)` bytes per interleaved row, or `ceil(cols * bits / 8)` per sample-plane row for a single band — instead of the unpacked one-byte-per-sample output; a full-width read reproduces the exact on-disk bytes. For byte-aligned depths (8/16/32/64-bit) the packed bytes are identical to the unpacked storage bytes, so a caller can use the packed path for all depths. Compression is still decompressed and reads stay bounded/windowed (per strip/tile); only the sub-byte bit-unpack is skipped. Implemented by re-packing the existing windowed unpacked decode (the exact inverse of the reader's sub-byte unpack), so the existing unpacked API and its behavior are unchanged
