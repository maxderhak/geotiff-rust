...
- guarantee (and regression-test) that `TiffWriter::write_block` LZW output is decodable by a strict TIFF LZW decoder. `write_block` with `Compression::Lzw` compresses each strip/tile in a single one-shot `weezl` `Encoder::encode` call, which emits the leading `ClearCode`, the code stream, and the trailing `EndOfInformation` marker — byte-for-byte identical to `into_stream().encode_all()`. This resolves the "Incremental `write_block` LZW interop" follow-on in `docs/ONYX-FORK-CHANGES.md` §5: the failure mode there is a non-finishing incremental encoder (a bare `encode_bytes` without `finish()`), which omits the end code and is rejected by strict decoders (including this fork's own reader, which requires `LzwStatus::Done`). The current one-shot path was never incremental and does not exhibit this; the change adds a unit test (`compress_lzw_matches_one_shot_encode_all_and_is_decodable`) that pins the output byte-identical to `into_stream().encode_all()` and asserts it decodes to completion, a doc-comment on `compress_lzw` recording the finished-one-shot contract, and an integration round-trip suite (`writeblock_lzw_roundtrip.rs`) covering halftone-shaped sub-byte (1/2/4-bit) chunky and planar-separate strips plus 8-bit, with fixtures large enough to cross the 9→10→11→12-bit code-size switches and force a `ClearCode`. No encoder behavior changed
